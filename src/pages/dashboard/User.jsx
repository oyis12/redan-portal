...
import userIcon from "../../assets/images/icons/user_outline.png";
import edit from "../../assets/images/icons/edit_outline.png";
import notificationImg from "../../assets/images/icons/notification.png";
import bin from "../../assets/images/icons/bin.png";
import plus from "../../assets/images/icons/plus.png";
import { ThreeDots } from "react-loader-spinner";
...
const User = () => {
  ...
  const handleDeleteUser = (record) => {
    confirm({
      ...
      icon: <ExclamationCircleFilled />,
      onOk() {
        setDataSource((prev) => prev.filter((user) => user.key !== record.key));
        message.success("User deleted");
      },
    });
  };

  const handleBlockUnblock = (record) => {
    const newStatus = record.status === "active" ? "blocked" : "active";
    const updated = dataSource.map((u) =>
      u.key === record.key ? { ...u, status: newStatus } : u
    );
    setDataSource(updated);
    notification.success({
      message: `User ${newStatus === "active" ? "Unblocked" : "Blocked"}`,
    });
  };
  ...
  const columns = [
    ...
    },
    {
      ...
      render: (status) => (
        <span
          className={
            status === "active"
              ? "bg-green-100 text-green-700 px-3 rounded-full"
              : "bg-red-100 text-red-600 px-3 rounded-full"
          }
        >
          {status.charAt(0).toUpperCase() + status.slice(1)}
        </span>
    ...
    },
    {
      ...
      render: (_, record) => (
            ...
            items: [
              {
                key: "view",
                label: (
                  <Link to={`/user/${record.key}`} state={{ record }}>
                    <span className="flex items-center">
                      <img src={userIcon} className="w-4 mr-2" alt="" />
                      View Profile
                    </span>
                  </Link>
                ),
              },
              {
                ...
                label: (
                  ...
                  >
                    <img src={no_data} className="w-4 mr-2" alt="" />
                    {record.status === "active" ? "Block" : "Unblock"}
                  </span>
                ),
              },
              {
                ...
                label: (
                  <span className="flex items-center">
                    <img src={notificationImg} className="w-4 mr-2" alt="" />
                    Send Notification
                  </span>
                ),
              },
              {
                ...
                label: (
                  ...
                  >
                    <img src={bin} className="w-4 mr-2" alt="" />
                    Delete
                  </span>
                ),
              },
        ...
        >
          <Button>
            <img src={dots} alt="menu" className="w-1" />
          </Button>
        </Dropdown>
      ),
    },
  ...
  return (
    <div className="relative top-14">
      <div className="bg-white rounded p-4 overflow-x-auto">
        ...
        {loading ? (
          <div className="flex justify-center items-center h-64">
            <ThreeDots height="80" width="80" color="#F1B31C" visible />
          </div>
        ) : (
          <Table
            columns={columns}
      ...
      >
        ...
        >
          ...
          >
            <Input prefix={<PhoneOutlined />} placeholder="8012345678" />
          </Form.Item>
